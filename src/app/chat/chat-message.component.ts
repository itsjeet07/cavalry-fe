...
@Component({
  ...
  template: `
    <div class="avatar" [style.background-image]="avatarStyle" *ngIf="!reply && !callLog">
      <ng-container *ngIf="!avatarStyle">
        {{ getInitials() }}
      </ng-container>
    </div>
    <div class="message" [ngClass]="{'m-0': callLog}" #list>
      <ng-container [ngSwitch]="type">

        <nb-chat-message-file *ngSwitchCase="'file'"
                              [sender]="sender" [date]="date" [dateFormat]="dateFormat"
                              [message]="message" [files]="files" [messageObj]="messageObj"
                              (onDeleteMessage)="onDeleteMessage.emit($event)"
                              (replyParticularMessage)="replyParticularMessage.emit($event)"
                              (onScrollElement)="onScrollElement.emit($event)">
        ...
        <nb-chat-message-text *ngSwitchDefault
                              [callLog]="callLog"
                              [uiColor]="uiColor" [sender]="sender" [date]="date" [dateFormat]="dateFormat"
                              [message]="message" [messageObj]="messageObj"
                              (onDeleteMessage)="onDeleteMessage.emit($event)"
                              (replyParticularMessage)="replyParticularMessage.emit($event)"
                              (onScrollElement)="onScrollElement.emit($event)">
