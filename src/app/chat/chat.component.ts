...
@Component({
    ...
    template: `
        ...
        <div [ngClass]="{'scrollable' : !callLog, 'call-height': callLog}" style="overflow-y: auto" #scrollable>
      ...
      <span class="reply-user-date">{{replyName}}, {{replyDate}}</span>
      <span class="close-btn" (click)="changeReplyFlag()">x</span>
    </span>
        <span class="reply-to bottom-reply" *ngIf="replyImageFlag">
      <blockquote><sup><i class="fa fa-quote-left" aria-hidden="true"></i></sup><img style="height:50px;width:50px"
                                                                                     [src]="replyToMessage"
                                                                                     alt="FILE Reply"></blockquote>
      <span class="reply-user-date">{{replyName}}, {{replyDate}}</span>
      <span class="close-btn" (click)="changeReplyImageFlag()">x</span>
    </span>
        <div class="form">
...
export class NbChatComponent implements OnChanges, AfterContentInit, AfterViewInit {
    ...
    @Input() replyDate;
    @Input() replyFlag;
    @Input() replyImageFlag;
    @Input() currentUser;
    @Output() replyFlagEmit = new EventEmitter();
    @Output() replyImageFlagEmit = new EventEmitter();
    /**
    ...
    changeReplyFlag() {
        ...
        this.replyFlagEmit.emit(this.replyFlag);
    }

    changeReplyImageFlag() {
        this.replyImageFlag = false;
        this.replyImageFlagEmit.emit(this.replyImageFlag);
    }
}
